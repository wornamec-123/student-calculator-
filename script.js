...
function addCourse() {
  ...
  row.innerHTML = `
    <input type="text"   placeholder="e.g. DCIT104" id="name-${courseCount}" />
    <input type="number" placeholder="3" min="1" max="6" id="credits-${courseCount}" value="3" />
    <input type="number" placeholder="Score" min="0" max="100" id="score-${courseCount}" />
...
function calculateGPA() {
  ...
  rows.forEach(row => {
    ...
    const name    = document.getElementById("name-"    + id).value || "Course";
    ...
    if (isNaN(score) || score < 0 || score > 100) {
      alert("Please enter a valid score (0–100) for all courses.");
      hasError = true;
      return;
    }
    ...
    const earned      = info.point * credits;
    totalPoints      += earned;
    ...
    resultsHTML += `
      <div class="result-row">
        ...
        <small>${credits} cr · ${score}% · ${earned.toFixed(1)} pts</small></span>
        <span class="grade-badge ${info.css}">${info.grade}</span>
      </div>`;
  });
  ...
  const degreeClass = getDegreeClass(gpa);

  document.getElementById("course-results").innerHTML = resultsHTML;
...
// Start with 3 courses by default
addCourse();
addCourse();
addCourse();
